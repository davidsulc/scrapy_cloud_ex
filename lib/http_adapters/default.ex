if Code.ensure_loaded?(:hackney) do
  defmodule ScrapyCloudEx.HttpAdapters.Default do
    ...
    defp format_api_result(status, %{message: message}),
      do: format_response_error(status, message)
  ...
  defmodule ScrapyCloudEx.HttpAdapters.Default do
    ...
    @spec request(any()) :: no_return()
    def request(_), do: raise()

    @spec handle_response(any(), any()) :: no_return()
    def handle_response(_, _), do: raise()
