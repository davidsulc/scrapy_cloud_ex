...
defmodule ScrapyCloudEx.HttpAdapters.DefaultTest do
  ...
  describe "handle_response/2" do
    test "generic success response", %{response: response} do
      ...
      {:ok, decoded_body} =
        @adapter.handle_response(%{response | status: 200, body: encoded_body}, @decoder_opts)
    ...
    test "400 Bad Request response in json format", %{response: response} do
      ...
      body =
        "{\"status\": \"badrequest\", \"message\": \"User 'johndoe' doesn't have access to project 123\"}"

      assert {:error, error} =
               @adapter.handle_response(%{response | status: 400, body: body}, @decoder_opts)

      assert Map.equal?(error, %{
               message: "User 'johndoe' doesn't have access to project 123",
               status: 400
             })
    end
    ...
    test "json response", %{response: response} do
      ...
      assert {:ok, decoded_body} =
               @adapter.handle_response(%{response | body: body}, @decoder_opts)
